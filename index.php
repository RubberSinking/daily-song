...
function playRadioIndex(){ if(!radioQueue.length) return; if(radioIndex >= radioQueue.length){ buildRadioQueue(); radioMeta.textContent='Reshuffled the archive'; }
  const t = radioQueue[radioIndex]; radioAudio.src = t.audio; radioNow.textContent = t.title; radioMeta.textContent = `${t.date} · ${radioIndex+1} of ${radioQueue.length} in current shuffle`; if(t.image){radioImage.src=t.image; radioImage.style.display='block';}else{radioImage.style.display='none'; radioImage.src='';} radioCaption.textContent = t.caption || ''; const bits=[]; if(t.lyrics) bits.push(`<a href="#" class="radio-open" data-kind="lyrics" data-title="${t.title} — Lyrics" data-src="${t.lyrics}">Lyrics</a>`); if(t.prompt) bits.push(`<a href="#" class="radio-open" data-kind="prompt" data-title="${t.title} — Prompt" data-src="${t.prompt}">Prompt</a>`); radioLinks.innerHTML = bits.join(' · '); radioLinks.querySelectorAll('.radio-open').forEach(a=>a.addEventListener('click', async e=>{ e.preventDefault(); overlayTitle.textContent=a.dataset.title||''; overlayBody.className='overlay-body '+(a.dataset.kind||''); overlayBody.textContent='Loading…'; overlay.classList.add('open'); try{ const r=await fetch(a.dataset.src,{cache:'no-store'}); const txt=await r.text(); overlayBody.textContent=txt; } catch(err){ overlayBody.textContent='Could not load.'; } })); radioAudio.play(); }
